...
class PagedeConnexion:  #classe formulaire 
    def __init__(self,root): #self représente l'instance de la classe. En utilisant le mot clé " self " nous pouvons accéder aux attributs et méthodes de la classe en python. 
        ...
        self.PagedeConnexion.iconbitmap("bibliothequeBIBYASO/Images/bib.ico")  #a modifier importer l'icone de l'application
        ...
        self.ImageDelaPageConnexion = PhotoImage(file="bibliothequeBIBYASO/Images/bibya.png") # logo a gauche des livres 
    ...
    def CliquerMotdepasseOublie(self): #fonction mot de passe oublié
        if self.emailDeLaPageConnexion.get()=="": #si le champs email est null alors
            messagebox.showerror("erreur", "Veuillez saisir un e-mail valide pour modifier votre mot de passe ", parent=self.PagedeConnexion)#on affiche un message d'erreur pour donner un mail valide
        else:
            try:
                ...
                if ligne == None: #si les champs ne corresponde pas a ceux dans la base de données alors 
                    ...
                    self.PageDeMotdepasseOublie.title("Mot de passe oublié") #titre de mot de passe oublie
                    self.PageDeMotdepasseOublie.config(bg="#ff7f00") #couleur du background 
                    ...
                    self.PageDeMotdepasseOublie.iconbitmap("bibliothequeBIBYASO/Images/bib.ico") 

                    #Grand titre
                    titreMotdepasseOublieDelaPageMotdepasseOublie = Label(self.PageDeMotdepasseOublie, text= "Rénitialisez votre mot de passe", font =("Arial", 12,"bold"), bg="#ff7f00",fg="black")
                    titreMotdepasseOublieDelaPageMotdepasseOublie.place(x=57, y=20) 
                    

                    titreSelectionnezunequestionDelaPageMotdepasseOublie = Label(self.PageDeMotdepasseOublie, text="Séléctionnez une question", font =("Arial", 10,"bold"),bg="#ff7f00",fg="black")
                    ...
                    repondreDelaPageMotdepasseOublie = Label(self.PageDeMotdepasseOublie, text="Répondre *", font =("Arial", 10,"bold"),bg="#ff7f00", fg="black")
                    repondreDelaPageMotdepasseOublie.place(x=10, y = 130)

                    # titres
                    nouveaumotdepasseDelaPageMotdepasseOublie = Label(self.PageDeMotdepasseOublie, text="Nouveau Mot de passe *", font =("Arial", 10,"bold"), bg="#ff7f00",fg="black")
                    nouveaumotdepasseDelaPageMotdepasseOublie.place(x=10, y = 180)

                    nouveaumotdepasseDelaPageMotdepasseOublie = Label(self.PageDeMotdepasseOublie, text="Confirmée Mot de passe *", font =("Arial", 10,"bold"), bg="#ff7f00",fg="black")
                    ...
                    self.titreRepondreDelaPageMotdepasseOublie.place(x=10, y =150)

                    self.motdepasseUtilisateurDelaPageMotdepasseOublie = Entry(self.PageDeMotdepasseOublie,show="*", font=(5), bg="white")
                    self.motdepasseUtilisateurDelaPageMotdepasseOublie.place(x=10, y =210)
                    #show="*" permet de rendre le mot de passe invisible 

                    self.confirmemotdepasseDelaPageMotdepasseOublie = Entry(self.PageDeMotdepasseOublie, show="*",font= (5), bg="white")
                    self.confirmemotdepasseDelaPageMotdepasseOublie.place(x=10, y =270)
                  
                    
                    
                    #bouton pour modifier mot de passe
                    BoutonMotdePasseOublieDeLaPageMotDepasseOublie = Button(self.PageDeMotdepasseOublie, text=" Modifier le mot de passe. ",command=self.MotdePasseOublie,cursor="hand2", font=("times new roman",11), bd=0,bg="white",fg="black")
                    BoutonMotdePasseOublieDeLaPageMotDepasseOublie.place(x=98, y=305)

                    
                    
            except Exception as ex:
                 messagebox.showerror("Erreur", f"erreur de connexion{str(ex)}", parent=self.CliquerBoutonCreeuncompte)
                 #cette ligne de code permet de gérer les exceptions qui se produisent pendant l'exécution du code et de 

    
    
    #Fonction de mot de passe oublié
    def MotdePasseOublie(self):
        if self.questionDeLaPageMotDepasseOublie.get()=="" or self.titreRepondreDelaPageMotdepasseOublie.get()=="" or self.motdepasseUtilisateurDelaPageMotdepasseOublie.get()=="" or self.confirmemotdepasseDelaPageMotdepasseOublie.get()=="": #si les champs ne sont pas rempli
            messagebox.showerror("Erreur","Veuillez remplir tous les champs obligatoires pour continuer.",parent=self.PageDeMotdepasseOublie) #alors affiche un message box pour dire remplir tout les champs
        elif self.motdepasseUtilisateurDelaPageMotdepasseOublie.get()!= self.confirmemotdepasseDelaPageMotdepasseOublie.get():#si les mots de passe ne sont pas pareils
            messagebox.showerror("Erreur", "Les mots de passe ne correspondent pas. Veuillez vérifier que les deux champs sont identiques.", parent=self.PageDeMotdepasseOublie) #
        else:
            #try permet d'encapsuler un code qui peut potentiellement générer des exceptions et de les gérer de manière appropriée, afin d'éviter les plantages du programme et fournir à l'utilisateur un message d'erreur clair et précis.
            try:
                ...
                connectionbdd = pymysql.connect(host=host, user=user, password=password, database=database)
                execut = connectionbdd.cursor()
                execut.execute("select * from utilisateur where emailUtilisateur=%s and questionUtilisateur=%s and reponseUtilisateur=%s",( self.emailDeLaPageConnexion.get(), self.questionDeLaPageMotDepasseOublie.get(), self.titreRepondreDelaPageMotdepasseOublie.get()))
                ligne= execut.fetchone()

                if ligne == None: #si cest faux alors affiche message box  
                   messagebox.showerror("Erreur", "Vous n'avez pas bien répondu à la question séléctionnez",parent= self.PageDeMotdepasseOublie)
                
                else:  # si cest correct alors
                    
                    motdepasse = self.motdepasseUtilisateurDelaPageMotdepasseOublie.get()
                    confirmemotdepasse = self.confirmemotdepasseDelaPageMotdepasseOublie.get()

                    execut.execute("UPDATE utilisateur SET motdepasseUtilisateur = %s, confirmemotdepasseUtilisateur = %s WHERE emailUtilisateur = %s", (motdepasse, confirmemotdepasse, self.emailDeLaPageConnexion.get()))
                    
                    connectionbdd.commit()
                    connectionbdd.close() # pour fermer la base de donnes
                    
                    messagebox.showinfo("Success","Votre mot de passe a été modifié avec")
    ...
    def PageCreationDeCompte(self): 
                    ...
                    self.PageCreationDeCompte.iconbitmap("bibliothequeBIBYASO/Images/bib.ico") 
    ...
    def OuvrirLaPageGestionLivres(self): #On declare une fonction pour ouvrir la page gestion livres
        ...
        call(["python", "bibliothequeBIBYASO/Gestionlivres.py"])# et avec le call on ouvre la page dans la quelle on souhaite aller
